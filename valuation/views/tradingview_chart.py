...
def render_tradingview_widget(ticker: str, height: int = 700, theme: str = "dark", key_prefix: str = "main") -> None:
    ...
    with col2:
        custom_ticker = st.text_input("Đổi mã cổ phiếu:", value=clean_ticker, key=f"tv_sym_input_clean_{key_prefix}_{ticker}")
        if custom_ticker:
    ...
    with st.spinner("Đang tải dữ liệu giá lịch sử từ vnstock..."):
        try:
            ...
            df = vnstock_client.get_historical_prices(clean_ticker, start_date)
